...
Tsifier.prototype.getCompiledFile = function (tsFile) {
	...
	if (!output) {
		...
		// try to find with realpath
		var srcFile = _(self.host.files).filter(function (f) {
				return getRelativeFilename(fs.realpathSync(f.filename)) === tsFile;
			}).map('filename').valueOf();

		if (srcFile.length === 1) {
			normalized = ts.normalizePath(srcFile[0]);
			jsFile = tsToJs(normalized);
